...
In this task, you will ask Cowork to classify the files in your OneDrive Lab Files folder and propose a consistent YYYY-MM-DD_Topic_DocType renaming scheme. Once you review and approve the plan, Cowork renames the files and organizes them into topic-based subfolders.

1. In the Cowork pane, enter the below **prompt** and click the **send** button:

    ```
    Analyze the files in my OneDrive "Lab Files" folder. Classify each by type and topic, then propose a renaming scheme of the form YYYY-MM-DD\_\<Topic\>\_\<DocType\> based on each file's content and metadata for the Project Orion business documents and trackers. Show me the full old-name → new-name mapping before renaming anything.
    ```
...
1. In the message box, enter the following **prompt (1)** and click **Submit (2)** to approve the proposed rename operation and continue to the next step.
    ...
    ```text
    Proceed with the suggested renaming, create a new folder for all latest renamed files, then also create subfolders by topic within it and move the files accordingly.
    ```
...
1. Verify Copilot Cowork begins executing the approved rename plan. Click on **Approve** to approve the folder creation and execute batch rename.

    ![](./media/m5e1t3s2.0.png)

1. Review the **folder (1)** creation and Click on **Approve (2)** to approve the request.
...
1. Review the Responsible AI guidance and proceed.
...
1. Enter the following **prompt (1)** and click on **send (2)** button.
    ...
    ```
    Analyze all files in my Project Orion Documents folder. Categorize documents into Sales, Procurement, Security, and Executive. Rename files using the format YYYY-MM-DD_Department_DocumentType and create subfolders for each category. Show me the proposed changes before making any modifications.
    ```
...
1. Enter the following **prompt (1)** and click **Send (2)** to update the governance workbook.
    ...
    ```
    Add a Business Function column to the File Governance Tracker workbook using this classification, then proceed with the file renaming and folder reorganization above.
    ```
